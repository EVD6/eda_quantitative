...
server <- function(input, output, session) {
  ...
  datasets <- reactive({
    if (input$data_source == "Upload Own File") {
      ...
      if (input$available_file == "stunting") {
        return(data.frame(stunting))
      } else if (input$available_file == "IPM") {
        return(data.frame(IPM))
      }
    }
  })
  ...
  output$plot <- renderPlot({
    ...
    if (input$pl == "Histogram")
      hist(dataset[, input$variable], col = 'gray', main = paste("Histogram of", input$variable), xlab = input$variable, ylab = "Frequency")
    else if (input$pl == "Boxplot")
      boxplot(dataset[, input$variable], col = 'gray', main = paste("Boxplot of", input$variable), horizontal = TRUE)
